...
When the verdict is `empty` or `partial`, the CLI, the developer HTML report and the plain-language report all state who the finding affects: not the people who chose to turn JavaScript off, but the people whose scripts never arrived (weak mobile network, corporate proxy, misbehaving browser extension, timeout). The wording is qualitative and carries no statistic, and is localised in all nine languages (`cli.noscript_impact`). It is advisory text and does not change the score.
