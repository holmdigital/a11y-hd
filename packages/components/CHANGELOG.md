...
- f73caa5: Intern #63 och #66 — `AccessibilityStatement` bar hela defektuppsättningen från Intern #64, som lagades i motorn men aldrig här.
...
- 46a8ed8: Intern #63 — `AccessibilityStatement` följer de rättade tillsynsmyndigheterna.
...
- 316a441: Add React 19 compatibility. Fixed three `cloneElement`/ref typing issues (Accordion, Select, Tooltip) that only surfaced under React 19's stricter types. Runtime behavior is unchanged: the full component suite passes under both React 18 and React 19 (649 tests on each).
